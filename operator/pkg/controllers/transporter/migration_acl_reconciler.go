...
func addDeployingSourceHub(needed map[string]struct{}, mcm *migrationv1alpha1.ManagedClusterMigration) {
	...
	// GH 1.4 uses legacy migration.from/to events without phase-based Deploying workflow.
	// gh-migration topic infrastructure is provisioned, but Write ACL grants are not
	// driven by ManagedClusterMigration status phase on this release line.
	_ = needed
	_ = mcm
}
