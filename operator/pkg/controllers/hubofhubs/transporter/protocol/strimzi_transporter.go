...
// AppendACLs add the simpleACLs to kafkauser, return true is it's acls is updated
func AppendACLs(
	kafkaUser *kafkav1beta2.KafkaUser,
	simpleACLs []kafkav1beta2.KafkaUserSpecAuthorizationAclsElem,
) bool {
	...
	for _, simpleACL := range simpleACLs {
		exist := false
		for _, acl := range authorization.Acls {
			if equality.Semantic.DeepEqual(acl, simpleACL) {
				exist = true
			}
		}
		if !exist {
			pendingACLs = append(pendingACLs, simpleACL)
			klog.Infof("ACL: %s -> %v -> %s/%s\n", kafkaUser.Name, simpleACL.Operations, simpleACL.Resource.Type,
				*simpleACL.Resource.Name)
		}
	}

	if len(pendingACLs) > 0 {
		kafkaUser.Spec.Authorization.Acls = append(authorization.Acls, pendingACLs...)
		return true
	}
	return false
}
