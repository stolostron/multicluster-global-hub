...
import (
	...
	commontran "github.com/stolostron/multicluster-global-hub/pkg/transport"
)
...
func (r *TransportReconciler) Reconcile(ctx context.Context, mgh *v1alpha4.MulticlusterGlobalHub) (err error) {
	// set the transporter
	var trans commontran.Transporter
	...
	if config.GetKafkaResourceReady() && r.kafkaController == nil {
		r.kafkaController, err = transporter.StartKafkaController(r.Manager, r.reconcileKafkaResources)
		if err != nil {
			return err
		}
	}
...
func (r *TransportReconciler) reconcileKafkaResources(ctx context.Context, mgh *v1alpha4.MulticlusterGlobalHub) error {
	...
	trans, err := transporter.NewStrimziTransporter(
		r.GetClient(),
		mgh,
		transporter.WithContext(ctx),
		transporter.WithCommunity(operatorutils.IsCommunityMode()),
	)
	if err != nil {
		return err
	}
	...
	err = trans.CreateAndUpdateUser(transporter.DefaultGlobalHubKafkaUser)
	if err != nil {
		return err
	}
	...
	topics := trans.GenerateClusterTopic(transporter.GlobalHubClusterName)
	err = trans.CreateAndUpdateTopic(topics)
	if err != nil {
		return err
	}
	// grant permission: read - status,event, write - spec;
	err = trans.GrantRead(transporter.DefaultGlobalHubKafkaUser, topics.EventTopic)
	if err != nil {
		return err
	}
	err = trans.GrantRead(transporter.DefaultGlobalHubKafkaUser, topics.StatusTopic)
	if err != nil {
		return err
	}
	err = trans.GrantWrite(transporter.DefaultGlobalHubKafkaUser, topics.SpecTopic)
	if err != nil {
		return err
	}
...
func loadTransportConn(ctx context.Context, trans commontran.Transporter) error {
	...
	err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 10*time.Minute, true,
		func(ctx context.Context) (bool, error) {
			conn, err = trans.GetConnCredential(transporter.DefaultGlobalHubKafkaUser)
			if err != nil {
