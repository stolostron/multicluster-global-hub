...
func (r *StatusReconciler) Reconcile(ctx context.Context, mgh *v1alpha4.MulticlusterGlobalHub,
	...
	readyReason := condition.CONDITION_REASON_GLOBALHUB_READY
	readyMessage := condition.CONDITION_MESSAGE_GLOBALHUB_READY
	if reconcileErr != nil {
		readyStatus = metav1.ConditionFalse
		readyReason = condition.CONDITION_REASON_GLOBALHUB_FAILED
	...
	if err == nil {
		...
		if err := condition.SetConditionDataRetention(ctx, r.Client, mgh, condition.CONDITION_STATUS_TRUE, msg); err != nil {
			return err
		}
	} else {
