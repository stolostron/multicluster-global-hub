...
// The URI must include explicit username and password so pgx cannot fill them from PG* env vars or .pgpass.
func spiceDBPostgresConfig(databaseURI string) (*pgx.ConnConfig, error) {
	postgresURI, err := parsePostgresURI(databaseURI)
	if err != nil {
		return nil, err
	}
	if postgresURI.User == nil || postgresURI.User.Username() == "" {
		return nil, fmt.Errorf("postgres connection is missing a username")
	}
	if _, err := postgresPasswordFromURI(postgresURI); err != nil {
		return nil, err
	}
