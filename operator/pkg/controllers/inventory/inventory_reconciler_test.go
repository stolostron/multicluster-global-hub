...
func TestSpiceDBPostgresConfig(t *testing.T) {
	...
	t.Run("uri without explicit username is rejected when PGPASSWORD is set", func(t *testing.T) {
		t.Setenv("PGPASSWORD", "env-password")

		_, err := spiceDBPostgresConfig("postgresql://postgres.example:5432/hoh")
		require.Error(t, err, "spiceDB URI without explicit username must be rejected")
		assert.Equal(t, "postgres connection is missing a username", err.Error(),
			"pgx env fallback must not supply missing username")
	})

	t.Run("uri without explicit password is rejected when PGPASSWORD is set", func(t *testing.T) {
		t.Setenv("PGPASSWORD", "env-password")
