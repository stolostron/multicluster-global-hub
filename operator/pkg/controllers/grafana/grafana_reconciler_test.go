...
func TestNetworkPolicyPred_Grafana(t *testing.T) {
	ns := utils.GetDefaultNamespace()
	grafanaName := config.COMPONENTS_GRAFANA_NAME

	np := &networkingv1.NetworkPolicy{
		ObjectMeta: metav1.ObjectMeta{Name: grafanaName, Namespace: ns},
	}
	otherNP := &networkingv1.NetworkPolicy{
		ObjectMeta: metav1.ObjectMeta{Name: "other", Namespace: ns},
	}

	assert.True(t, networkPolicyPred.Create(event.CreateEvent{Object: np}), "Create: grafana NP should match")
	assert.False(t, networkPolicyPred.Create(event.CreateEvent{Object: otherNP}), "Create: other NP should not match")
	assert.True(t, networkPolicyPred.Update(event.UpdateEvent{ObjectNew: np}), "Update: grafana NP should match")
