...
// validateSingleCluster validates a single cluster and returns validation result with error message and condition reason
...
func (m *ClusterMigrationController) validateClustersForMigration(
	...
	for _, cluster := range mcm.Spec.IncludedManagedClusters {
		...
		if singleResult := validateSingleCluster(cluster, clusterToClusterInfoMap, mcm); singleResult.ErrorMessage != "" {
			...
			// Set condition reason based on validation result - use the first error type encountered
		}
	}
	log.Infof("%v clusters verify failed", failedClustersCount)

	if failedClustersCount > 0 {
		// Set the condition reason based on the failure type
		condition.Reason = ConditionReasonClusterValidateFailed
...
func getclusterToClusterInfoMap(mcm *migrationv1alpha1.ManagedClusterMigration) (map[string]ManagedClusterInfo, error) {
	...
	err := db.Select("cluster_name", "leaf_hub_name", "payload->'metadata'->'annotations' as annotations FROM status.managed_clusters").
		Where("payload->'metadata'->>'name' IN (?) AND deleted_at IS NULL", mcm.Spec.IncludedManagedClusters).
		Find(&records).Error
	if err != nil {
