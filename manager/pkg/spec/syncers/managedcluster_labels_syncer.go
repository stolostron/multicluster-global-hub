...
func syncManagedClusterLabelsBundles(ctx context.Context, producer transport.Producer, transportBundleKey string,
	...
	for leafHubName, managedClusterLabelsBundle := range leafHubToLabelsSpecBundleMap {
		...
		log.Debugf("send leafhub label events: %v, i: %v, j:%v", transportBundleKey, leafHubName, managedClusterLabelsBundle.LeafHubName)
		for _, v := range managedClusterLabelsBundle.Objects {
			log.Debug("#########obj: %v", *v)
			log.Debug("#########obj: %v", v.Labels)
		}
