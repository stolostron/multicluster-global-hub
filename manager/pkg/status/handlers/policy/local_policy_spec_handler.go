...
func RegisterLocalPolicySpecHandler(c client.Client, conflationManager *conflator.ConflationManager) {
...
func (h *localPolicySpecHandler) handleEvent(ctx context.Context, evt *cloudevents.Event) error {
	...
	if configs.IsInventoryAPIEnabled() {
		...
		if err != nil {
			h.log.Warnf("failed to get version from cluster claim: %v", err)
		}
