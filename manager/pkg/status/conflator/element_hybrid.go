...
func (e *hybridElement) Predicate(eventVersion *version.Version) bool {
	if eventVersion.InitGen() {
		...
		log.Infow("resetting element processed version", "version", eventVersion)
	}
	log.Debugw("inserting event", "version", eventVersion)
	return eventVersion == e.elementState.LastProcessedVersion ||
		eventVersion.NewerThan(e.elementState.LastProcessedVersion)
}
