...
var _ = Describe("SubscriptionReportsDbSyncer", Ordered, func() {
	const (
		leafHubName = "hub1"
		testSchema  = database.StatusSchema
		testTable   = database.SubscriptionReportsTableName
		messageKey  = constants.SubscriptionReportMsgKey
	)
	...
	It("sync the SubscriptionReport bundle", func() {
		By("Create SubscriptionReport bundle")
		...
		Eventually(func() error {
			...
			for rows.Next() {
				...
				if err := rows.Scan(&hubName, &appsubReport); err != nil {
					return err
				}
				if hubName == statusBundle.LeafHubName &&
					appsubReport.Name == statusBundle.Objects[0].GetName() {
					return nil
				}
			}
