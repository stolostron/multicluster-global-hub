...
// go test ./manager/pkg/statussyncer/syncers -v -ginkgo.focus "SubscriptionReportHandler"
var _ = Describe("SubscriptionReportHandler", Ordered, func() {
	It("should be able to sync placement decision event", func() {
		...
		evt := ToCloudEvent(leafHubName, string(enum.SubscriptionReportType), version, data)
		...
		Eventually(func() error {
			sql := fmt.Sprintf("SELECT leaf_hub_name,payload FROM %s.%s", database.StatusSchema,
				database.SubscriptionReportsTableName)
			...
			for rows.Next() {
				var hubName string
				var payload []byte
				app := &appsv1alpha1.SubscriptionReport{}
