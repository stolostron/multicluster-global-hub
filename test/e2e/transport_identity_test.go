...
func migrationDeployingEvent(sourceHub, targetHub, clusterName, migrationID string) cloudevents.Event {
	bundle := migrationbundle.SourceClusterMigrationResources{
...
func migrationInitializingEvent(
	sourceHub, targetHub, migrationID, managedServiceAccountName string,
) cloudevents.Event {
	bundle := migrationbundle.ManagedClusterMigrationToEvent{
		MigrationId:               migrationID,
		Stage:                     migrationv1alpha1.PhaseInitializing,
		ManagedServiceAccountName: managedServiceAccountName,
		SourceHubs:                []string{sourceHub},
	}
...
func seedInFlightMigrationState(
	...
	evt := migrationInitializingEvent(sourceHub, targetHub, migrationID, managedServiceAccountName)
	Expect(publisher.SendToTopic(ctx, publisher.SpecTopic(), evt)).To(Succeed(),
		"expected initializing migration event to seed in-flight migration state on target hub agent")
}
