...
func byoSharedTransportSecret() (*corev1.Secret, error) {
	...
	defer cancel()
	return testClients.KubeClient().CoreV1().Secrets(GlobalhubNamespace).Get(
...
func createBYOPerHubSecret(clusterName string, mutate func(*corev1.Secret)) {
	...
	kube := testClients.KubeClient().CoreV1().Secrets(GlobalhubNamespace)
	apiCtx, cancel := byoAPIContext()
	defer cancel()
...
func deleteBYOPerHubSecret(clusterName string) {
	apiCtx, cancel := byoAPIContext()
	defer cancel()
	err := testClients.KubeClient().CoreV1().Secrets(GlobalhubNamespace).Delete(
...
func managedHubAgentKafkaConfig(hubClient client.Client) (*transport.KafkaConfig, error) {
	...
	cfg, err := transportconfig.GetKafkaCredentailBySecret(secret, hubClient)
	if err != nil {
...
func operatorLogsContain(substr string) error {
	apiCtx, cancel := byoAPIContext()
	defer cancel()
	pods, err := testClients.KubeClient().CoreV1().Pods(GlobalhubNamespace).List(apiCtx, metav1.ListOptions{
	...
	for i := range pods.Items {
		logs, logErr := testClients.KubeClient().CoreV1().Pods(GlobalhubNamespace).
