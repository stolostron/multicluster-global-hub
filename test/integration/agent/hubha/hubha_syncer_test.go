...
var _ = Describe("Hub HA Standby Syncer Integration", func() {
	...
	It("should not overwrite the standby's own local-cluster ManagedCluster (ACM-41773)", func() {
		...
		clientConfigs, found, _ := unstructured.NestedSlice(got.Object, "spec", "managedClusterClientConfigs")
		Expect(found && len(clientConfigs) > 0).To(BeFalse(),
			"standby local-cluster must not receive the active hub's client configs")

		accepts, _, _ := unstructured.NestedBool(got.Object, "spec", "hubAcceptsClient")
