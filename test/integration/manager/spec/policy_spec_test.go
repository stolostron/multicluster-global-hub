...
var _ = Describe("Policy controller", Ordered, func() {
	It("create the Policy in kubernetes", func() {
		...
		Expect(runtimeClient.Create(ctx, testPolicy, &client.CreateOptions{})).ToNot(HaveOccurred())
	})
	...
	It("update the policy from postgres", func() {
		...
		err := runtimeClient.Get(ctx, client.ObjectKeyFromObject(updatedPolicy), updatedPolicy)
		Expect(err).Should(Succeed())

		updatedPolicy.Spec.Disabled = false

		err = runtimeClient.Update(ctx, updatedPolicy)
		Expect(err).Should(Succeed())
	...
	It("delete the policy from postgres", func() {
		...
		err = runtimeClient.Delete(ctx, updatedPolicy, &client.DeleteOptions{})
		Expect(err).Should(Succeed())
