...
func deployGlobalHub() error {
	...
	if os.Getenv("IS_CANARY_ENV") != "true" {
		...
		Eventually(func() error {
			for i := 1; i <= ExpectedLeafHubNum; i++ {
				cmd := exec.Command("kubectl", "--context", "kind-hub"+strconv.Itoa(i), "apply", "-f", fmt.Sprintf("%s/0000_01_operator.open-cluster-management.io_multiclusterhubs.crd.yaml", testOptions.HubCluster.CrdsDir))
	...
	fmt.Println(testOptions.HubCluster.MghManagerImageREF)
	if testOptions.HubCluster.MghManagerImageREF != "" {
		cmd := exec.Command("sed", "-i", fmt.Sprintf("s|quay.io/stolostron/multicluster-global-hub-manager:latest|%s|g", testOptions.HubCluster.MghManagerImageREF), rootDir+"/operator/config/manager/manager.yaml")
		output, err := cmd.CombinedOutput()
		fmt.Println(string(output))
		if err != nil {
			fmt.Println(err)
	...
	fmt.Println(testOptions.HubCluster.MghAgentImageREF)
	if testOptions.HubCluster.MghAgentImageREF != "" {
		cmd := exec.Command("sed", "-i", fmt.Sprintf("s|quay.io/stolostron/multicluster-global-hub-agent:latest|%s|g", testOptions.HubCluster.MghAgentImageREF), rootDir+"/operator/config/manager/manager.yaml")
		output, err := cmd.CombinedOutput()
		fmt.Println(string(output))
		if err != nil {
			fmt.Println(err)
