...
ssh "${OPT[@]}" "$HOST" "cd $HOST_DIR && . test/scripts/env.list && sudo make e2e-dep && make e2e-setup" > >(tee "$ARTIFACT_DIR/e2e-setup.log") 2>&1

echo "runn e2e tests"
ssh "${OPT[@]}" "$HOST" "cd $HOST_DIR && . test/scripts/env.list && make e2e-test-all && make e2e-test-prune" > >(tee "$ARTIFACT_DIR/e2e-test.log") 2>&1
