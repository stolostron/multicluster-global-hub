...
These two jobs' status are saved in the metrics named `multicluster_global_hub_partition_job_status` and `multicluster_global_hub_compliance_job_status`, as shown in the figure below from the console of the Openshift cluster. Where `0` means the job runs successfully, otherwise `1` means failure.

![Global Hub Partition Job Status Metrics Panel](./images/global-hub-partition-job-status-panel.png)
![Global Hub Compliance Job Status Metrics Panel](./images/global-hub-compliance-job-status-panel.png)
