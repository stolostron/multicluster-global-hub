...
4. Ugrading issue from Global Hub 1.2 to 1.3(only for the BYO kafka). There are 3 topics named `spec`, `status` and `event` in your kafka cluster. However the kafka topics can be configured after upgrading it to 1.3. Notably, we will only keep 2 topics in the for the system: one for spec path and the other for status path. The default external topic will be `gh-spec` and `gh-status`. So ensure these are aligned between your Kafka clusters and the Global Hub operand. You can either create the two topics in your Kafka cluster or modify the operand to use the existing topics in your Kafka cluster.