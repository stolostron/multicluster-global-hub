...
4. When reimport a managed hub cluster with different name. The old managed hub data will be null in the following dashboards. `Global Hub - Ad Hoc Policy Investigation` `Global Hub - What's Changed / Clusters` and `Global Hub - What's Changed / Policies`.
5. When a managed hub is detached, all policies and applications created on managed hub will be deleted.
6. If managed hub running on Openshift 3.11, It can not redirect to the `RHACM Observability` page in `Global Hub - Overview` dashboard.