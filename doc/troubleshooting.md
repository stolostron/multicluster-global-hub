...
To aid in troubleshooting, you may need detailed log information from the operator, manager, and agent components. Currently, the supported log levels are `debug`, `info`, `warn`, and `error`. For the complete list of available log levels, refer to [zap log levels](https://github.com/uber-go/zap/blob/master/level.go#L30-L49).
