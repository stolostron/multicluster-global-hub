...
The [StackRox](https://www.stackrox.io) platform is a security solution, which helps detecting and
managing security risks and vulnerabilities. Its _Central_ component offers detailed information
about such security risks and vulnerabilities in the form of a collection of _security violations_.
The _Central_ web application includes a dashboard that shows a summary of those security violations
...
The feature is disabled by default. In order to enable it is necessary to configure the Global
Hub _multiclusterglobalhub_ object and the StackRox _Central_ objects.
...
The Global Hub Agent needs a API token to connect to each _Central_ instance. That token can be
created using the _Central_ web application. Go to _Platform Configuration_, _Integrations_ and
scroll down to _API Token_. In the dialog select a name, role and expiration date for the token,
then click the _Generate_ button:

![StackRox API Token Dialog](./images/stackrox-create-api-token-dialog.png)
...
![StackRox API Token Result](./images/stackrox-create-api-token-result.png)

Make sure to copy the token to a safe place, as you will not be able to see it again once you close
that dialog.

For more details about tokens check the [StackRox
...
trusted in order to connect to the _Central_ API server. In a typical StackRox installation that CA
...
metadata:
  ...
  name: stackrox-connection-details
stringData:
  token: eyJ...
  ca: |
    -----BEGIN CERTIFICATE-----
...
global-hub.open-cluster-management.io/with-stackrox-credentials-secret=multicluster-global-hub-agent/stackrox-connection-details
```
