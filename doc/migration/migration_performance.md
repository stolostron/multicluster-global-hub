...
- **Source Hub (Hub1)**: ACM version **2.15**
...
| Execution Model  | **Completed**: 300 clusters (1 forward migration) |
...
- **OpenShift GitOps Applications**:
  - **ztp-clusters-01**: Manages ~297 SNO clusters (vm00001-vm00297)
...
|--------------|---------------|---------------------|---------|
| ZTP Migration | 300 clusters  | 1 forward migration | ✅ 100% success |

> **Captured metrics:**
> - ✅ Migration time and phase breakdown
> - ✅ Post-migration policy convergence time
> - ✅ Post-migration ClusterInstance convergence time
...
**Test Configuration:**
- **Migration YAML**: [mcm-300.yaml](mcm-300.yaml)
...
| Resource Type | Convergence Time | Final State | Details |
|---------------|------------------|-------------|---------|
| **Policies** | **~6 minutes** | All Compliant | All 6 ZTP policies reached compliant state across all 300 clusters |
| **ClusterInstance** | **~2 minutes** | All Completed | All 300 ClusterInstance resources transitioned to Completed state |
| **GitOps Application** | N/A | Active | `ztp-clusters-01` successfully managing 300 clusters on target hub |

### 7.3 Key Findings

✅ **Excellent Post-Migration Performance:**
- Policy compliance achieved in **6 minutes** for 300 clusters 
- ClusterInstance completion in **2 minutes** for 300 clusters 
...
- Rapid convergence demonstrates efficient policy engine performance
...
| Direction | Total Time | Success Rate | Notes |
|-----------|-----------|--------------|-------|
| Forward (local-cluster → hub2) | 4 minutes 40 seconds | 100% | Full completion |
---
...
- Total migration time: **4 minutes 40 seconds** for 300 clusters
- Average time per cluster: **~0.93 seconds**
- All migration phases completed within configured timeout

✅ **Post-Migration Convergence**: **Exceptional**
- **Policies**: All 6 ZTP policies reached compliant state in **~6 minutes** 
- **ClusterInstance**: All 300 resources transitioned to Completed state in **~2 minutes**
...
- ✅ **Rapid Policy Convergence**: All policies compliant within 6 minutes post-migration
...
- ✅ **100% Success Rate**: No errors or failures during migration
...
This test demonstrates that GlobalHub migration is production-ready for large-scale ZTP deployments with comprehensive error handling and recovery capabilities.
