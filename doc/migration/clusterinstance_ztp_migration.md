...
|-----------|------------|------------|-------|
| **ACM** | Version N | Version N to N+2 | Supports up to 2 versions skew |
...
The target hub must be configured with the **same components** as the source hub to ensure successful cluster migration.
...
The migration process consists of five main steps: pre-migration verification, creating the migration resource, monitoring progress, GitOps configuration migration, and cleanup.
...
#### 2.2 Dynamic Cluster Selection with Placement
...
#### 3.2 Monitor Individual Cluster Status
...
### 4. Migrate GitOps Configuration to Target Hub

> **Critical Step**: ClusterInstance resources are managed by GitOps (Argo CD) and are **NOT automatically migrated**. You must manually configure GitOps applications on the target hub.

#### 4.1 Understanding GitOps in ZTP

In ZTP deployments, clusters are typically managed through two types of Argo CD applications:

1. **Policy Applications**: Manage ACM policies applied to clusters
2. **ClusterInstance Applications**: Manage ClusterInstance resources, secrets, and cluster-specific configurations

Both must be redeployed on the target hub for proper cluster management.

#### 4.2 Export GitOps Applications from Source Hub
**Step 1**: Export Policy Applications
...
**Step 2**: Export ClusterInstance Applications
...
#### 4.3 Apply GitOps Applications to Target Hub

> **Note**: Execute these commands on the **target hub** cluster.
**Step 1**: Apply Policy Applications
```bash
# Apply policy applications
oc apply -f policies-app.yaml
```

**Step 2**: Apply ClusterInstance Applications
```bash
# Apply ClusterInstance applications
oc apply -f clusterinstance-app.yaml
```
#### 4.4 Verify ClusterInstance Status on Target Hub
...
#### 4.6 Verify Managed Cluster Connection
...
#### 5.1 Remove GitOps Applications
...
#### 5.2 Delete ClusterInstance Resources
...
#### 5.3 Verify Related Resources are Cleaned Up
...
└────────┬────────┘              └────────┬────────┘              └────────┬────────┘
         ...
         │                                │  2. Import Target Hub          │
         │                                │  ══════════════════════════════>
         │                                │  - Import as managed hub       │
         │                                │  - Add hosted-mode label       │
         │                                │                                │
         │  [MANUAL]                      │                                │
         │  3. Create Migration           │                                │
         │  <══════════════════════════════                                │
         │  - Apply ManagedClusterMigration CR                             │
         │  - Specify clusters to migrate │                                │
         │                                │                                │
         │                                │                                │
         │  [AUTO]                        │                                │
         │  4. Migrate Resources and Register Clusters                    │
         │  ───────────────────────────────────────────────────────────────>
         │                                │                                │
         │  [AUTO]                        │                                │
         │  5. Pause & Disconnect         │                                │
         │  ───────────────────────────────>                               │
         │  - Pause cluster reconcile     │                                │
         │  - Disconnect manageclusters   │                                │
         │                                │                                │
         │                                │                                │
         │  [MANUAL]                      │                                │
         ...
         │  - Delete source GitOps apps   │                                │
         │  - Delete ClusterInstances     │                                │
         │                                │                                │
         ▼                                ▼                                ▼
