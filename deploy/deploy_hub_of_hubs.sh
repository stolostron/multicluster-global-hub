...
curl -s "https://raw.githubusercontent.com/open-cluster-management/hub-of-hubs-spec-transport-bridge/$TAG/deploy/hub-of-hubs-spec-transport-bridge.yaml.template" | SYNC_SERVICE_PORT="$sync_service_port" IMAGE="nirrozenbaumibm/hub-of-hubs-spec-transport-bridge:latest" envsubst | kubectl apply -f - -n "$acm_namespace"
curl -s "https://raw.githubusercontent.com/open-cluster-management/hub-of-hubs-status-transport-bridge/$TAG/deploy/hub-of-hubs-status-transport-bridge.yaml.template" | SYNC_SERVICE_PORT="$sync_service_port" IMAGE="nirrozenbaumibm/hub-of-hubs-status-transport-bridge:latest" envsubst | kubectl apply -f - -n "$acm_namespace"
