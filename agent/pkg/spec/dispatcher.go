...
func (d *genericDispatcher) dispatch(ctx context.Context) {
	for {
		...
		case evt := <-d.consumer.EventChan():
			...
			if !specEventSourceAllowed(ctx, d.client, d.agentConfig, evt, subject) {
				d.log.Warnw("event dropped due to untrusted source",
					"type", evt.Type(), "source", evt.Source(), "subject", subject)
				continue
			}
			d.mu.RLock()
