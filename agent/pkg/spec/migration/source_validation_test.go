...
func TestIsMigrationDeployResourceAllowed(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:        "foreign cluster deployment name denied",
			resource:    foreignClusterDeployment,
			clusterName: "cluster1",
			allowed:     false,
		},
		{
			name:        "foreign cluster deployment namespace denied",
			resource:    foreignNamespaceClusterDeployment,
			clusterName: "cluster1",
			allowed:     false,
		},
		{name: "image cluster install allowed", resource: imageClusterInstall, clusterName: "cluster1", allowed: true},
		{
			name:        "foreign image cluster install name denied",
			resource:    foreignImageClusterInstall,
			clusterName: "cluster1",
			allowed:     false,
		},
		{
			name:        "foreign image cluster install namespace denied",
			resource:    foreignNamespaceImageClusterInstall,
			clusterName: "cluster1",
			allowed:     false,
		},
		{name: "host firmware settings allowed", resource: hostFirmwareSettings, clusterName: "cluster1", allowed: true},
		{
			name:        "foreign host firmware settings denied",
			resource:    foreignHostFirmwareSettings,
			clusterName: "cluster1",
			allowed:     false,
		},
		...
		{
			name:        "foreign host firmware components denied",
			resource:    foreignHostFirmwareComponents,
			clusterName: "cluster1",
			allowed:     false,
		},
