...
// If there are more than maxErrorsToShow, it shows the first maxErrorsToShow and indicates how many remain.
func formatErrorMessages(errors map[string]string) string {
	...
	if len(errors) > maxErrorsToShow {
		return fmt.Sprintf("%v and %d more, get more details in events", result, len(errors)-maxErrorsToShow)
	}
